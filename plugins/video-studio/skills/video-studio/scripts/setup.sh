...
# video-use liest .env aus seinem Ordner ODER cwd -> Symlink auf die zentrale .env
ln -sf ../../.env engines/video-use/.env 2>/dev/null || cp -f .env engines/video-use/.env

# --- 3. video-use Python-Env (uv, lokal in engines/video-use/.venv) ---
echo "[setup] installiere video-use Python-Deps (uv)..."
( cd engines/video-use && uv venv --quiet && uv pip install -e . ) \
